...
//A Recursicve Helper Function to use it to for recursive calls
void helper(int mat[n][n], int r, int c, string path, bool vis[n][n]){
    ...
    //Base Condition: When rat is at the end
    if( r == n - 1 && c == n - 1){
        cout << path << endl;
        return;
    }

    //For Visiting the Paths marked as True
    vis[r][c] = true;

    //For traversing thorugh the mazehg
